...
# Run the app (serves frontend + API on :8090, per main()/uvicorn.run)
...
`authority-tracker.service` is a systemd unit that runs `main.py` from `/opt/Authority-Tracker` via the project's `.venv` — reference for how this is deployed in production, not something you normally need to touch.

## Architecture

### Backend (`main.py`, `models.py`, `schemas.py`, `database.py`)

- **`database.py`** — SQLite engine (`tracker.db`) + `SessionLocal`/`Base`. `get_db()` is the FastAPI dependency used by REST routes; Socket.IO handlers instead open their own `with database.SessionLocal() as db:` block since they aren't part of the FastAPI DI chain.
...
The frontend logic was split from a single `script.js` into native ES modules under `js/`, loaded via a single `<script type="module" src="js/main.js">` in `index.html` (no bundler/build step — the browser resolves the relative `import`s directly):
...
- `js/dom.js` — cached DOM element references (`const`s), imported by whichever module needs them.
...
- `js/identity.js` — ensures `localStorage.getItem('user')` is set (prompted once on first load) *before* `socket.js` joins the shared `"tts"` room with it; imported as `socket.js`'s first import specifically to guarantee that ordering.
- `js/game.js` — core gameplay logic: building tracker widgets, `handleRotate`/`updateWidgetDimension` (see rotation note below), authority adjustment, danger/warning states, endgame sequence.
- `js/menu.js` — the menu overlay and its panels (names, rotate, battle log, match history, stats), including the REST fetches for `/api/current_log`, `/api/games`, `/api/stats`.
- `js/main.js` — entry point: boots state from `/api/state`, wires up the setup-screen listeners, and (via side-effect imports) pulls in `socket.js`/`menu.js` so their event listeners register.
- `js/toast.js` — thin wrapper around the globally-loaded Toastify library.
...
- Layout is mobile-first and must keep working when a `tracker-widget` is rotated 90/180/270° (`handleRotate`/`updateWidgetDimension` in `js/game.js`) to match players seated on different sides of one device; a `ResizeObserver` keeps rotated widgets correctly sized.

### Static assets

`media/` (images, sound, video, third-party JS libs) is mounted at `/media` via `StaticFiles`; `js/` (this app's own ES modules) is likewise mounted at `/js` via `StaticFiles`. `index.html`/`style.css` are still served by explicit FastAPI routes rather than a generic static mount.
