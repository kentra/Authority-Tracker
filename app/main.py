...
# Frontend lives in static/ at the project root (this file is app/main.py).
STATIC_DIR = Path(__file__).resolve().parent.parent / "static"

app.mount("/media", StaticFiles(directory=STATIC_DIR / "media"), name="media")
app.mount("/js", StaticFiles(directory=STATIC_DIR / "js"), name="app_js")
...
def main():
    ...
    # Run from the project root as `uv run python -m app.main` so this import
    # string and the reloader's module discovery both resolve correctly.
    uvicorn.run("app.main:socket_app", host="0.0.0.0", port=8090, reload=True)
