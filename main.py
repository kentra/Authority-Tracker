...
async def request_status_report(sid):
    ...
    with database.SessionLocal() as db:
        ...
        prompt = f"You are a narrator commenting on the current game. You will answer as if you were a character in the game Star Realms in a imersive way. Give a dramatic 2-sentence status report with insults towards the player with lowest Authority. Current standings: {scores_text}."
