...
async def request_status_report(sid):
    ...
    with database.SessionLocal() as db:
        ...
        prompt = f"You are a narrator commenting on the current game. You will answer as if you were a character in the game Star Realms in a imersive way. Give a dramatic 2-sentence status report while slightly mocking the player with lowest Authority and praising the player with the highest Authority. You should also announce current standings based on this context: {scores_text}."
